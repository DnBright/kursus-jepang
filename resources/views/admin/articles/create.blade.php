<x-admin-layout>
    <div class="max-w-[1400px] mx-auto space-y-6 pb-20 text-left">
        ...
        <div class="flex items-center justify-between">
            <h1 class="text-2xl font-bold text-slate-900">Add New Post</h1>
        ...
        <form action="{{ route('admin.articles.store') }}" method="POST" enctype="multipart/form-data" class="grid grid-cols-1 lg:grid-cols-12 gap-8">
            @csrf
            ...
            <div class="lg:col-span-8 space-y-6">
                ...
                <div class="bg-white rounded-xl border border-slate-200 shadow-sm overflow-hidden">
                    <input type="text" name="title" required value="{{ old('title') }}" placeholder="Enter title here" class="w-full px-6 py-4 text-xl font-medium border-none focus:ring-0 placeholder-slate-300">
                ...
                <!-- Editor Container -->
                <div class="bg-white rounded-xl border border-slate-200 shadow-sm overflow-hidden">
                    <textarea name="content" id="editor" class="w-full h-[600px]">{{ old('content') }}</textarea>
                ...
                <div class="bg-white rounded-xl border border-slate-200 shadow-sm overflow-hidden">
                    ...
                    <div class="p-6">
                        <textarea name="excerpt" rows="4" placeholder="Write a short summary..." class="w-full px-4 py-3 bg-white border border-slate-200 rounded-xl text-sm focus:outline-none focus:ring-2 focus:ring-red-500/10 focus:border-red-500 transition-all font-medium">{{ old('excerpt') }}</textarea>
                    ...
                    <div class="p-6 space-y-4">
                        ...
                        <div class="space-y-3 pt-2">
                            <div class="flex items-center gap-2 text-xs font-medium text-slate-600">
                                ...
                                <span>Status: <span class="font-bold text-slate-900">draft</span></span>
                            </div>
                            <div class="flex items-center justify-between">
                                <div class="flex items-center gap-2 text-xs font-medium text-slate-600">
                                ...
                                <label class="relative inline-flex items-center cursor-pointer">
                                    <input type="checkbox" name="is_member_only" value="1" {{ old('is_member_only') ? 'checked' : '' }} class="sr-only peer">
                                    <div class="w-11 h-6 bg-slate-200 peer-focus:outline-none rounded-full peer peer-checked:after:translate-x-full rtl:peer-checked:after:-translate-x-full peer-checked:after:border-white after:content-[''] after:absolute after:top-[2px] after:start-[2px] after:bg-white after:border-gray-300 after:border after:rounded-full after:h-5 after:w-5 after:transition-all peer-checked:bg-red-600"></div>
                                </label>
                            </div>
                            <div class="flex items-center gap-2 text-xs font-medium text-slate-600">
                                <svg class="w-4 h-4 text-slate-400" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M8 7V3m8 4V3m-9 8h10M5 21h14a2 2 0 002-2V7a2 2 0 00-2-2H5a2 2 0 00-2 2v12a2 2 0 002 2z"></path></svg>
                                <span>Publish: <span class="font-bold text-slate-900">Immediately</span></span>
                            </div>
                        </div>

                        <div class="pt-4 border-t border-slate-100 flex items-center justify-between">
                            <a href="{{ route('admin.articles.index') }}" class="text-xs font-bold text-red-600 hover:underline">Move to Trash</a>
                            <button type="submit" name="is_published" value="1" class="px-6 py-2 bg-blue-600 text-white font-bold rounded-lg text-xs hover:bg-blue-700 transition-colors shadow-lg shadow-blue-200">Publish</button>
                        </div>
                    </div>
                </div>
                ...
                <div class="bg-white rounded-xl border border-slate-200 shadow-sm overflow-hidden">
                    ...
                    <div class="p-6 space-y-4">
                        <div class="aspect-video w-full bg-slate-50 border-2 border-dashed border-slate-200 rounded-2xl flex flex-col items-center justify-center gap-3 group hover:border-red-500 transition-all cursor-pointer relative overflow-hidden" onclick="document.getElementById('image-input').click()">
                            ...
                            <svg class="w-10 h-10 text-slate-300 group-hover:text-red-500 transition-colors" id="image-placeholder-icon" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M4 16l4.586-4.586a2 2 0 012.828 0L16 16m-2-2l1.586-1.586a2 2 0 012.828 0L20 14m-6-6h.01M6 20h12a2 2 0 002-2V6a2 2 0 00-2-2H6a2 2 0 00-2 2v12a2 2 0 002-2z"></path></svg>
                            <p class="text-[10px] font-bold text-slate-400 uppercase tracking-widest group-hover:text-red-500 transition-colors" id="image-placeholder-text">No image selected</p>
    ...
    <script>
        ...
        function previewImageUrl(url) {
            if (url && url.trim() !== '') {
                ...
                if (!document.getElementById('image-input').files.length) {
