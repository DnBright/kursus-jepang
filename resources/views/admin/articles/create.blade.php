<x-admin-layout>
    <div class="max-w-4xl mx-auto space-y-6 pb-20 text-left">
        ...
        <div>
            ...
            <p class="text-slate-500 text-sm mt-1">Buat konten menarik untuk dibagikan kepada siswa.</p>
        </div>
            ...
            <div class="bg-white rounded-2xl border border-slate-200 shadow-sm overflow-hidden">
                ...
                <div class="p-6 space-y-6">
                    <div class="space-y-2">
                        ...
                        <input type="text" name="title" required value="{{ old('title') }}" placeholder="Masukkan judul yang menarik..." class="w-full px-4 py-2.5 bg-slate-50 border border-slate-200 rounded-xl text-sm focus:outline-none focus:ring-2 focus:ring-red-500/20 focus:border-red-500 transition-all font-medium">
                        @error('title') <p class="text-red-500 text-xs mt-1">{{ $message }}</p> @enderror
                    </div>

                    <div class="space-y-2">
                        <label class="text-xs font-bold text-slate-500 uppercase tracking-wider">Ringkasan (Excerpt)</label>
                        <textarea name="excerpt" rows="3" placeholder="Gambarkan singkat isi artikel ini..." class="w-full px-4 py-2.5 bg-slate-50 border border-slate-200 rounded-xl text-sm focus:outline-none focus:ring-2 focus:ring-red-500/20 focus:border-red-500 transition-all font-medium">{{ old('excerpt') }}</textarea>
                    ...
                    <div class="space-y-2">
                        ...
                        <p class="text-slate-400 text-[10px]">Format: JPG, PNG, GIF. Maks: 2MB.</p>
                        @error('image') <p class="text-red-500 text-xs mt-1">{{ $message }}</p> @enderror
                    </div>

                    <div class="space-y-2">
                        <label class="text-xs font-bold text-slate-500 uppercase tracking-wider">Isi Artikel</label>
                        <textarea name="content" required rows="15" placeholder="Tuliskan isi artikel lengkap di sini..." class="w-full px-4 py-2.5 bg-slate-50 border border-slate-200 rounded-xl text-sm focus:outline-none focus:ring-2 focus:ring-red-500/20 focus:border-red-500 transition-all font-medium">{{ old('content') }}</textarea>
                        @error('content') <p class="text-red-500 text-xs mt-1">{{ $message }}</p> @enderror
                    </div>

                    <div class="flex items-center gap-2 bg-slate-50 p-4 rounded-xl border border-slate-200">
                        <input type="checkbox" name="is_published" value="1" {{ old('is_published') ? 'checked' : '' }} class="w-4 h-4 text-red-600 focus:ring-red-500 border-slate-300 rounded cursor-pointer">
