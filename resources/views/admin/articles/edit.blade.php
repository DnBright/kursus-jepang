                    ...
                    <div class="p-6">
                        ...
                        <script>
                            function deleteCategory(name, el) {
                                if (confirm('Hapus kategori ini? Semua artikel dengan kategori ini akan diubah menjadi tanpa kategori.')) {
                ...
                <div class="bg-white rounded-xl border border-slate-200 shadow-sm overflow-hidden">
                    ...
                    <div class="p-6 space-y-4">
                        <div class="aspect-video w-full bg-slate-50 border-2 border-dashed border-slate-200 rounded-2xl flex flex-col items-center justify-center gap-3 group hover:border-red-500 transition-all cursor-pointer relative overflow-hidden" onclick="document.getElementById('image-input').click()">
                            <img id="image-preview" src="{{ $article->image ? (str_starts_with($article->image, 'http') ? $article->image : Storage::url($article->image)) : '' }}" class="absolute inset-0 w-full h-full object-cover {{ $article->image ? '' : 'hidden' }}">
    ...
    <script>
        ...
        function previewImageUrl(url) {
            if (url && url.trim() !== '') {
                ...
                // If URL cleared, maybe hide or show placeholder
                // But only if no file selected too
