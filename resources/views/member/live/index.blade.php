<x-app-layout>
    <div class="space-y-8">
        ...
        <div class="grid grid-cols-1 lg:grid-cols-2 gap-6">
                    ...
                    <div class="flex items-center justify-between border-t border-slate-100 pt-4 mt-auto">
                        <div class="flex items-center gap-2 text-sm text-slate-600 font-medium">
                            <div class="w-6 h-6 rounded-full bg-slate-900 text-white flex items-center justify-center text-[10px] font-bold">
                                {{ substr($session->instructor->name, 0, 1) }}
                            </div>
            ...
            @empty
                @if(!$liveNow)
                <div class="col-span-2 text-center py-12 bg-white rounded-3xl border border-dashed border-slate-300">
                    <div class="text-6xl mb-4">📅</div>
                    <h3 class="text-xl font-bold text-slate-900 mb-2">Belum Ada Jadwal</h3>
                    <p class="text-slate-500 max-w-sm mx-auto">Kami sedang menyiapkan jadwal kelas seru untuk Anda.</p>
                </div>
                @endif
            @endforelse
            ...
            <div class="bg-white rounded-2xl p-6 border border-slate-200 hover:shadow-md transition-all group flex flex-col md:flex-row gap-6 opacity-75 hover:opacity-100">
                ...
                <div class="flex-1">
                    <div class="flex justify-between items-start mb-2">
                        <span class="px-2.5 py-0.5 rounded-md bg-slate-100 text-slate-500 text-xs font-bold uppercase tracking-wider">{{ $session->course->title }}</span>
                        <span class="text-sm font-medium text-slate-400">Selesai</span>
                    </div>
                    <h4 class="text-lg font-bold text-slate-700 mb-2">{{ $session->title }}</h4>
                    <p class="text-slate-400 text-sm mb-4 line-clamp-2">{{ $session->description ?? 'Sesi telah berakhir.' }}</p>
                    ...
                    <div class="flex items-center justify-between border-t border-slate-100 pt-4 mt-auto">
                        ...
                        <a href="{{ $session->recording_url }}" target="_blank" class="px-4 py-2 bg-white border border-slate-200 text-slate-600 hover:bg-slate-50 font-bold rounded-lg text-sm flex items-center gap-2 transition-colors">
                            <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M14.752 11.168l-3.197-2.132A1 1 0 0010 9.87v4.263a1 1 0 001.555.832l3.197-2.132a1 1 0 000-1.664z"></path><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M21 12a9 9 0 11-18 0 9 9 0 0118 0z"></path></svg>
                            Tonton Rekaman
                        </a>
                        @else
                        <span class="text-xs text-slate-400 italic">Rekaman tidak tersedia</span>
                        @endif
                    </div>
                </div>
            </div>
