            ...
            @forelse($roadmapSteps as $step)
                ...
                <div class="bg-white rounded-2xl shadow-sm border border-slate-200 p-4 flex items-center justify-between group cursor-move roadmap-item transition-all hover:shadow-md hover:border-slate-300" data-id="{{ $step->id }}">
                    ...
                    <div class="flex gap-2 items-center pl-4 border-l border-slate-100">
                        ...
                        @endif
                        <form action="{{ route('sensei.roadmap.destroy', $step->id) }}" method="POST" onsubmit="return confirm('Apakah Anda yakin ingin menghapus kotak ini secara permanen?')">
                            @csrf
                            @method('DELETE')
    ...
    <script>
        document.addEventListener('alpine:init', () => {
            Alpine.data('roadmapForm', () => ({
                isOpen: false,
                isEdit: false,
                formAction: '{{ route('sensei.roadmap.store', $course->id) }}',
                formData: {
                    title: '',
                    block_type: 'materi',
                ...
                openAddModal() {
                    this.isEdit = false;
                    this.formAction = '{{ route('sensei.roadmap.store', $course->id) }}';
                    this.formData = {
                        title: '',
                        block_type: 'materi',
                        quiz_type: 'multiple_choice',
                ...
                openEditModal(data) {
                    this.isEdit = true;
                    this.formAction = '/sensei/roadmap/steps/' + data.id;
