<x-sensei-layout>
    <div class="max-w-4xl mx-auto">
        <div class="mb-8 flex items-center justify-between">
            <div>
                <h2 class="text-2xl font-bold text-slate-900">Edit Tugas</h2>
                <p class="text-slate-500 text-sm mt-1">Ubah detail dan instruksi tugas.</p>
            </div>
        ...
        <div class="bg-white rounded-2xl border border-slate-200 shadow-sm overflow-hidden mb-8" x-data="{ courseId: '{{ $assignment->module->course_id }}' }">
            <form action="{{ route('sensei.assignments.update', $assignment->id) }}" method="POST" class="p-8 space-y-6">
                @csrf
                @method('PUT')
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    ...
                    <div class="md:col-span-2 space-y-2">
                        ...
                        <input type="text" name="title" id="title" required value="{{ old('title', $assignment->title) }}" 
                    ...
                    <div class="space-y-2">
                        ...
                        <select name="course_id" id="course_id" required x-model="courseId"
                            ...
                            <option value="{{ $course->id }}" {{ $assignment->module->course_id == $course->id ? 'selected' : '' }}>{{ $course->title }}</option>
                            @endforeach
                        </select>
                    </div>
                    ...
                    <div class="space-y-2 shadow-sm rounded-xl">
                        ...
                        <select name="module_id" id="module_id" required 
                            ...
                            <option value="{{ $module->id }}" {{ $assignment->module_id == $module->id ? 'selected' : '' }}>{{ $module->title }}</option>
                            @endforeach
                        </select>
                    </div>
                    ...
                    <div class="space-y-2">
                        ...
                        <select name="assignment_type" id="assignment_type" required 
                            class="w-full px-4 py-3 rounded-xl border border-slate-200 focus:border-blue-500 focus:ring-4 focus:ring-blue-500/10 transition-all font-medium appearance-none">
                            <option value="writing" {{ $assignment->assignment_type == 'writing' ? 'selected' : '' }}>Writing / Menulis</option>
                            <option value="speaking" {{ $assignment->assignment_type == 'speaking' ? 'selected' : '' }}>Speaking / Berbicara</option>
                            <option value="reading" {{ $assignment->assignment_type == 'reading' ? 'selected' : '' }}>Reading / Membaca</option>
                            <option value="listening" {{ $assignment->assignment_type == 'listening' ? 'selected' : '' }}>Listening / Mendengar</option>
                            <option value="project" {{ $assignment->assignment_type == 'project' ? 'selected' : '' }}>Project / Tugas Akhir</option>
                        </select>
                    </div>
                    ...
                    <div class="space-y-2">
                        ...
                        <input type="number" name="max_score" id="max_score" required value="{{ old('max_score', $assignment->max_score) }}" min="1"
                            class="w-full px-4 py-3 rounded-xl border border-slate-200 focus:border-blue-500 focus:ring-4 focus:ring-blue-500/10 transition-all font-medium">
                    </div>
                    ...
                    <div class="space-y-2">
                        ...
                        <input type="datetime-local" name="due_date" id="due_date" value="{{ $assignment->due_date ? $assignment->due_date->format('Y-m-d\TH:i') : '' }}"
                    ...
                    <div class="md:col-span-2 space-y-2">
                        ...
                        <textarea name="description" id="description" rows="5" required
                            class="w-full px-4 py-3 rounded-xl border border-slate-200 focus:border-blue-500 focus:ring-4 focus:ring-blue-500/10 transition-all font-medium">{{ old('description', $assignment->description) }}</textarea>
                    </div>

                    <!-- Is Required -->
                    <div class="md:col-span-2 flex items-center gap-3 p-4 bg-blue-50 rounded-xl border border-blue-100">
                        <input type="checkbox" name="is_required" id="is_required" value="1" {{ $assignment->is_required ? 'checked' : '' }}
        ...
        <div class="bg-red-50 rounded-2xl border border-red-100 p-8 flex items-center justify-between gap-6">
            <div>
                <h3 class="text-red-900 font-bold">Hapus Tugas</h3>
                <p class="text-red-600 text-sm mt-1">Seluruh data tugas dan submisi siswa akan dihapus secara permanen.</p>
            </div>
            <form action="{{ route('sensei.assignments.destroy', $assignment->id) }}" method="POST" onsubmit="return confirm('Apakah Anda yakin ingin menghapus tugas ini secara permanen?');">
                @csrf
                @method('DELETE')
